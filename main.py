...
# para que sea la primera en ejecutarce @app.before_request
@app.before_request
def middleware():
    ...
    if (urlCliente == "/login"):
        ...
        if infoToken["rol"] is not None:
            tienePersmiso = validarPermiso(urlCliente, metodoCliente, infoToken["rol"]["_id"])
...
def validarPermiso(urlCliente,metodoCliente,idRol):
    ...
    response = requests.get(urlValidarPermiso,json=bodyRequest, headers=headers)
    try:
        data=response.json()
        if("_id" in data):
            tienePermiso=True
